...
const Newsletter: React.FC<NewsletterProps> = ({ bgColor, btnColor}) => {
  return (
    <>
      <div className={`${
        bgColor && bgColor !== "" ? bgColor : "bg-customKhakiDark"
      } flex items-center justify-center py-5 md:py-10 lg:py-20`}>
        <div style={{paddingRight:'0'}} className="container bg-white mx-auto flex justify-center items-center w-full shadow-xl border-0 lg:rounded-xl">
          ...
          <button className={`${
            btnColor && btnColor !== "" ? btnColor : "bg-customGreen"
          } text-left text-white px-2 lg:px-6 py-2 lg:py-5 text-xs md:text-base lg:text-xl lg:rounded-r-xl w-5/12`}>Sign Up for Newsletters</button>
        </div>
      </div>
    </>
  )              
